...
module.exports = {
    ...
    RowExists: function (tableRow, connection) {
        ...
        return new Promise((resolve, reject) => {
            ...
            var query = "SELECT * FROM " + tableRow.table + " WHERE " + this.BuildSelectionKey(tableRow.columns);
            //console.log(query);
    ...
    InsertRow: function (tableRow, connection) {
        ...
        return new Promise((resolve, reject) => {
            ...
            var query =
                "INSERT INTO " + tableRow.table + " " + insertValues + " " +
                "ON DUPLICATE KEY UPDATE `ctPlays` = `ctPlays` + 1";

            connection.query(query, function (err, result) {
                if (err) {
                    if (err.code == "ER_DUP_ENTRY") {
                        console.log("Duplicate row: " + tableRow.table + " " + insertValues);
                        resolve(-1);
                    }
                    else {
    ...
    ReadRecents: function (inputFilename) {
