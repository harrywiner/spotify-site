...
select trackName, count(*) as times from plays where msPlayed >= 10000 group by trackName order by times asc;
...
select trackName, count(*) as times from plays where msPlayed >= 10000 group by trackName order by times desc LIMIT 10;
